...
const BoxShape: React.FC<BoxShapeProps> = ({ variant, className, lineColor, layout }) => {
  const paths = {
    ...
    >
      {layout === "grid" ? (
        <div className="grid grid-cols-2 gap-4 md:gap-6 w-[80%] md:w-[60%] lg:w-[70%] max-w-[500px] opacity-60">
          <BoxShape variant="left" className="w-full h-auto" lineColor={lineColor} layout={layout} />
          <BoxShape variant="top" className="w-full h-auto" lineColor={lineColor} layout={layout} />
          <BoxShape variant="bottom" className="w-full h-auto" lineColor={lineColor} layout={layout} />
          <BoxShape variant="right" className="w-full h-auto" lineColor={lineColor} layout={layout} />
        </div>
      ) : (
